...
class Page(db.Model):
	...
	def by_subject(cls, subject):
		return Page.all().filter('subject =', subject).get()
...
class EditPage(WikiHandler):
	def get(self, subject):
		if self.user:
			p = Page.by_subject(subject)
			if not p:
				p = Page(subject=subject, content='')
			self.render('page.html', page=p, edit=True)
		else:
			self.redirect('/login')
	...
	def post(self, subject):
		if self.user:
			...
			p = Page.by_subject(subject)
			if not p:
				p = Page(subject=subject, content=content)
			else:
				p.content = content
			p.put()
